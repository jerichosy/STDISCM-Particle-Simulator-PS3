...
public class Particle {
    ...
    public void update(double deltaTime) {
        ...
        } else if (nextX >= Server.WINDOW_WIDTH) {
            angle = 180 - angle;
            nextX = Server.WINDOW_WIDTH; // Correct position
        }

        if (nextY <= 0) {
            angle = 360 - angle;
            nextY = 0; // Correct position to stay within bounds
        } else if (nextY >= Server.WINDOW_HEIGHT) {
            angle = 360 - angle;
            nextY = Server.WINDOW_HEIGHT; // Correct position
        }
